...
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      $('#advancedIgnoreRequestsForm').form({
        ...
        onSuccess: (event) => {
          ...
          if (!preferences.value.ignoreRequests.includes(excludeDomainPattern.value)) {
            preferences.value.ignoreRequests.push(excludeDomainPattern.value);
          }
    ...
    const resetStorage = async (): Promise<void> => {
      if (!window.confirm(`Wipe storage and reset it to default?\nThis can't be undone.`)) {
        return;
      }
      ...
      if (!reset) {
        this.$root.$emit(
          'showError',
          `Storage reset failed${resetError ? `: ${resetError}` : ''}`
        );
      } else {
    ...
    const removeIgnoredDomain = (ignoredPattern: string): void => {
      preferences.value.ignoreRequests = preferences.value.ignoreRequests.filter(
