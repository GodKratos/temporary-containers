...
import { PreferencesSchema, Tab, Debug, OnBeforeRequestResult, WebRequestOnBeforeRequestDetails } from '~/types';
...
export class Request {
  ...
  async webRequestOnBeforeRequest(request: WebRequestOnBeforeRequestDetails): Promise<OnBeforeRequestResult> {
    ...
    try {
      ...
      this.debug('[webRequestOnBeforeRequest] handling request failed', (error as Error).toString());
    }
    ...
    if (typeof returnVal === 'object' && returnVal.cancel) {
      ...
      return returnVal;
    } else if (!returnVal || (typeof returnVal === 'object' && !returnVal.clean)) {
  ...
  async handleRequest(request: WebRequestOnBeforeRequestDetails): Promise<OnBeforeRequestResult> {
    if (request.tabId === -1) {
    ...
    if (tab && tab.cookieStoreId !== `${this.background.containerPrefix}-default`) {
      try {
        ...
        if (container) {
          ...
          this.debug('[handleRequest] onBeforeRequest found a missing container without catching an exception', tab, request);
          return false;
        }
      } catch (error) {
        this.debug(
  ...
  cancelRequest(request: WebRequestOnBeforeRequestDetails): boolean {
    if (!request || typeof request.requestId === 'undefined' || typeof request.tabId === 'undefined') {
  ...
  shouldCancelRequest(request: WebRequestOnBeforeRequestDetails): boolean {
    ...
    if (
      ...
      (this.canceledTabs[request.tabId] &&
        (this.canceledTabs[request.tabId].requestIds[request.requestId] || this.canceledTabs[request.tabId].urls[request.url]))
    ) {
      return true;
    }
    return false;
